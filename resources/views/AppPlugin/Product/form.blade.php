          ...
          <div class="col-lg-9">
            <x-admin.card.normal>
              <div class="row">
                ...
                @if(config('AppPlugin.Product.brand'))
              ...
              @if(config('AppPlugin.Product.tags'))
                <div class="row">
                  <x-admin.form.select-multiple name="tag_id" :categories="$tags" :sel-cat="$selTags" col="12" :label="__('admin/proProduct.pro_text_tags')"/>
                </div>
              @endif
              <hr>
              <div class="row mt-2">
              ...
              <x-admin.card.normal>
                <ul class="child_product_name">
                  @foreach($rowData->childproduct as $childproduct )
                    <li>{{\App\AppPlugin\Product\ProductController::getChildProductName($childproduct)}} <span>{{number_format($childproduct->price)}}</span></li>
                  @endforeach
                </ul>
                <x-admin.form.action-button url="{{route('admin.Shop.Product.manage-attribute',$rowData->id)}}" :print-lable="__('admin/proProduct.pro_variant_manage')" :tip="false"/>
